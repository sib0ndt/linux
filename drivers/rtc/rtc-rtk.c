...
#include <soc/realtek/rtk_ptime.h>

#define RTK_RTC_READ_TIME_MAX_RETRY      3
...
static void rtk_rtc_restore_from_ptime(struct rtk_rtc_device *rdev)
{
	int ret;
	time64_t time = 0;
	struct rtc_time tm = {};

	ret = rtk_ptime_load_time(rdev->dev, &time);
	if (ret)
		return;

	rtc_time64_to_tm(time, &tm);
	dev_info(rdev->dev, "restore time from ptime-reg: %04d.%02d.%02d %02d:%02d:%02d",
		1900 + tm.tm_year, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour,
		tm.tm_min, tm.tm_sec);

	rtk_rtc_time_to_reg(rdev, RTK_RTC_TIME, time);
}

static void rtk_rtc_save_to_ptime(struct rtk_rtc_device *rdev)
{
	struct rtc_time tm;
	time64_t time;
	int ret;
	...
	ret = rtk_ptime_store_time(rdev->dev, time);
	if (ret)
		return;

	rtc_time64_to_tm(time, &tm);
	dev_info(rdev->dev, "save time to ptime-reg: %04d.%02d.%02d %02d:%02d:%02d",
		1900 + tm.tm_year, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour,
		tm.tm_min, tm.tm_sec);
}
...
static inline int rtk_rtc_get_sec(struct rtk_rtc_device *rdev)
{
	unsigned int second = rtk_rtc_reg_read(rdev, MIS_RTCSEC);

	return rdev->cfg->is_drtc ? second : second / 2;
}

static void rtk_rtc_reg_to_time(struct rtk_rtc_device *rdev, int type, time64_t *time)
{
	unsigned long flags;
	unsigned int day = 0, hour = 0, minute = 0, second = 0;
	unsigned int second_old = 0;
	int retries = RTK_RTC_READ_TIME_MAX_RETRY;
	...
	case RTK_RTC_TIME:
		...
		while (retries--) {
			second_old = rtk_rtc_get_sec(rdev);
			...
			second     = rtk_rtc_get_sec(rdev);

			if (second >= second_old)
				break;

			dev_dbg(rdev->dev, "second carried, read again: sec=%d,%d\n",
				second, second_old);
		}
		break;
...
static int rtk_rtc_time_to_reg(struct rtk_rtc_device *rdev, int type, time64_t time)
{
	...
	unsigned int max_day = rdev->cfg->is_drtc ? 0x7fff : 0x3fff;

	if (time < rdev->base_time)
		return -EINVAL;
	time -= rdev->base_time;

	day = div_u64_rem(time, 86400, &hour);
	hour = div_u64_rem(hour, 3600, &minute);
	minute = div_u64_rem(minute, 60, &second);
	...
	case RTK_RTC_TIME:
		if (rdev->cfg->is_drtc){
			rtk_rtc_reg_write(rdev, MIS_RTCSEC,   second);
			rtk_rtc_reg_write(rdev, MIS_RTCMIN,   minute);
			rtk_rtc_reg_write(rdev, MIS_RTCHR,    hour);
			rtk_rtc_reg_write(rdev, MIS_RTCDATE1, day);
		} else {
...
static void rtk_rtc_init(struct rtk_rtc_device *rdev)
{
	...
	if (rdev->use_hsec)
		rtk_rtc_reg_write(rdev, MIS_RTCCR, MIS_DRTCCR_HSUINTE);
...
static int rtk_rtc_parse_dt_config(struct device_node *np, struct rtk_rtc_device *rdev)
{
	...
	ret = of_property_read_u32(np, "rtc-base-year", &base_year);
	if (ret)
		base_year = 1900;
	...
	rdev->use_hsec = of_property_present(np, "use-hsec");

	if (!rdev->cfg->has_acr2)
		return 0;

	ret = of_property_read_u32(np, "rtc-bias", &rdev->bias);
	if (ret)
		rdev->bias = 2;
...
static int rtk_rtc_probe(struct platform_device *pdev)
{
	...
	rtk_rtc_restore_from_ptime(rdev);
...
static void rtk_rtc_shutdown(struct platform_device *pdev)
{
	struct rtk_rtc_device *rdev = platform_get_drvdata(pdev);
...
static int rtk_rtc_suspend(struct device *dev)
{
	struct rtk_rtc_device *rdev = dev_get_drvdata(dev);
	unsigned int val;
...
static int rtk_rtc_resume(struct device *dev)
{
	struct rtk_rtc_device *rdev = dev_get_drvdata(dev);
	unsigned int val;
...
static const struct dev_pm_ops rtk_rtc_pm_ops = {
	.suspend	= rtk_rtc_suspend,
	.resume		= rtk_rtc_resume,
	.freeze		= rtk_rtc_suspend,
	.thaw		= rtk_rtc_resume,
	.restore	= rtk_rtc_resume,
};
