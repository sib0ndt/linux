...
/*
 * irq_domain_add_simple() was removed from newer mainline kernels.
 * Vendor RTD code used OF node API; mainline 7.x wants fwnode API.
 */
static inline struct irq_domain *rtk_irq_domain_add_simple(struct device_node *of_node,
	unsigned int size,
	unsigned int first_irq,
	const struct irq_domain_ops *ops,
	void *host_data)
{
	return irq_domain_create_simple(of_fwnode_handle(of_node), size,
	...
	struct device_node *parent)
{
	 ...
	 */
	rtk_domain = rtk_irq_domain_add_simple(np,
		(nr_irq * IRQ_INMUX),
		160,
		&mux_irq_domain_ops,
		mux_data);
