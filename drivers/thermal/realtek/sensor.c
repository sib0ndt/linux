				     ...
				     const struct thermal_sensor_desc *desc)
{
	...
	tdev->tz = devm_thermal_of_zone_register(dev, 0, tdev, &rtk_thermal_device_ops);
	if (IS_ERR(tdev->tz)) {
...
static inline int is_vaild_temp(int temp, const struct thermal_sensor_desc *desc)
{
        int max_temp = 150000;
        int min_temp = -3000;
...
static int thermal_sensor_get_temp(struct thermal_zone_device *tz, int *temp)
{
	...
	if (ret || !is_vaild_temp(*temp, tdev->desc)) {
