...
static int clk_pll_determine_rate(struct clk_hw *hw, struct clk_rate_request *req)
{
	struct clk_pll *pll = to_clk_pll(hw);
	const struct freq_table *ftblv = NULL;

	if (WARN_ON_ONCE(req->best_parent_rate != default_osc_rate))
		return -EINVAL;

	ftblv = ftbl_find_by_rate(pll->freq_tbl, req->rate);
	if (!ftblv)
		return -EINVAL;
...
static int clk_pll_div_determine_rate(struct clk_hw *hw, struct clk_rate_request *req)
{
	struct clk_pll_div *plld = to_clk_pll_div(hw);
	...
	/* lookup div in dtbl */
	dv = dtbl_find_by_rate(plld->div_tbl, req->rate);
	if (WARN(!dv, "%pC: invalid d-tbl entry for %lu", hw->clk, req->rate))
		return -EINVAL;

	target_rate = req->rate * dv->div;

	/* mimic the old nested clk_pll_round_rate logic */
	ftblv = ftbl_find_by_rate(plld->clkp.freq_tbl, target_rate);
	if (!ftblv)
		return -EINVAL;

	req->rate = ftblv->rate / dv->div;
	return 0;
}
...
const struct clk_ops clk_pll_div_ops = {
	.debug_init       = clk_pll_debug_init,
	.determine_rate   = clk_pll_div_determine_rate,
...
#endif /* Disable unused _dif_ functions */
