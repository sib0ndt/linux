...
static dma_addr_t dma_phy_addr;
...
#if IS_ENABLED(CONFIG_CMD25_WO_STOP_COMMAND)
struct task_struct *rtk_SD_task;
...
static struct scatterlist *sdmmc_alloc_sg(int sg_len, gfp_t gfp)
{
...
void rtk_sdmmc_wait_stop_cmd_irq_done(void)
{
	...
	if ( istimeout ) {
		pr_err("****** WAIT wait_stop_command_irq_done TIMEOUT(%d,%d))\n",
...
static void rtk_sdmmc_pad_pwrctrl(struct rtk_sdmmc_host *rtk_host, int voltage)
{
	...
	reg_sel3318 = (readl(pll_base+CR_PLL_SD1) & 0x00006000) >> 13;

	switch (voltage) {
	case MMC_SIGNAL_VOLTAGE_330:
		if (reg_sel3318 == 0x1) {
			pr_err("[MSG] %s 1E0 = %x reg_sel3318 = %x keep 3.3v\n",
				__func__, readl(pll_base+CR_PLL_SD1), reg_sel3318);
		} else if (reg_sel3318 == 0x3){
			writel((readl(pll_base+CR_PLL_SD1) & 0xffffdfff) , pll_base+CR_PLL_SD1);
			mdelay(1);
			writel((readl(isopad_base+0x78) | 0x02001000) , isopad_base+0x78);
			writel((readl(isopad_base+0x7c) | 0x00080040) , isopad_base+0x7c);
			writel((readl(isopad_base+0x80) | 0x02001000) , isopad_base+0x80);
			mdelay(1);
			writel((readl(pll_base+CR_PLL_SD1) & 0xFFFF9FFF) , pll_base+CR_PLL_SD1);
			mdelay(1);
			writel((readl(pll_base+CR_PLL_SD1) | 0x00002000) , pll_base+CR_PLL_SD1);
			pr_err("[MSG] %s 1E0 = %x reg_sel3318 = %x switch 1.8v to 3.3v\n",
				__func__, readl(pll_base+CR_PLL_SD1), reg_sel3318);
		}
		break;
	case MMC_SIGNAL_VOLTAGE_180:
		if (reg_sel3318 == 0x1) {
			writel((readl(pll_base+CR_PLL_SD1) & 0xffff9fff) , pll_base+CR_PLL_SD1);
			mdelay(1);
			writel((readl(pll_base+CR_PLL_SD1) | 0x00004000) , pll_base+CR_PLL_SD1);
			mdelay(1);
			writel((readl(pll_base+CR_PLL_SD1) | 0x00006000) , pll_base+CR_PLL_SD1);
			mdelay(1);
			writel((readl(isopad_base+0x78) & 0xfdffefff) , isopad_base+0x78);
			writel((readl(isopad_base+0x7c) & 0xfff7ffbf) , isopad_base+0x7c);
			writel((readl(isopad_base+0x80) & 0xfdffefff) , isopad_base+0x80);
			pr_err("[MSG] %s 1E0 = %x reg_sel3318 = %x switch 3.3v to 1.8v\n",
				__func__, readl(pll_base+CR_PLL_SD1), reg_sel3318);
		} else if (reg_sel3318 == 0x3){
			pr_err("[MSG] %s 1E0 = %x reg_sel3318 = %x keep 1.8v\n",
				__func__, readl(pll_base+CR_PLL_SD1), reg_sel3318);
		}
		break;
	default:
		pr_err("[MSG] %s 1E0 = %x reg_sel3318 = %x invalid argument\n",
			__func__, readl(pll_base+CR_PLL_SD1), reg_sel3318);
	}
}
...
void remove_sdcard(struct rtk_sdmmc_host *rtk_host)
{
	...
	lock_fail = down_trylock(&cr_sd_sem); //check if the lock is occupied, if not, then occupied
	up(&cr_sd_sem); //release the lock

	if (chip_id == CHIP_ID_RTD1619B) {
		rtk_sdmmc_pad_pwrctrl(rtk_host, MMC_SIGNAL_VOLTAGE_330);
		/* Set pad driving */
		tmp = ((readl(isopad_base + 0x78) & 0xfe07f03f) | 0x00d806c0);
		writel(tmp, isopad_base + 0x78);
		tmp = ((readl(isopad_base + 0x7c) & 0xfff81fc0) | 0x0003601b);
		writel(tmp, isopad_base + 0x7c);
		tmp = ((readl(isopad_base + 0x80) & 0xfe07f03f) | 0x00d806c0);
		writel(tmp, isopad_base+0x80);
	...
	} else if (chip_id == CHIP_ID_RTD1619) {
	...
	} else if (chip_id == CHIP_ID_RTD1395) {
...
static void rtk_sdmmc_shutdown(struct platform_device *pdev)
{
	...
	pr_err("[SD] Realtek SD card reader shutdown!!\n");
...
static int __maybe_unused rtk_sdmmc_pm_suspend(struct device *dev)
{
	int ret = 0;
	u32 det_time = 0;
...
static int __maybe_unused rtk_sdmmc_pm_resume(struct device *dev)
{
	int ret = 0;
	u32 reginfo = 0;
	u32 det_time = 0;
	...
	if (reginfo & 0x8) {
		...
		if (chip_id == CHIP_ID_RTD1319) {
			...
			while((readl(rtk_host->sdmmc + 0xb4) & 0x80000000) == 0)
				pr_info("wait until clk open...\n");
		}
	...
	if(re_initialize) {	//this case is to handle that SD card is plugged during suspend
...
static u8 rtk_sdmmc_search_final_phase(struct rtk_sdmmc_host *rtk_host, u32 phase_map)
{
	...
	if (cont_path_cnt == 0) {
		pr_debug(" %s No continuous phase path\n", __func__);
		goto finish;
	} else {
	...
	for (i = 0 ; i < cont_path_cnt ; i++) {
		...
		pr_debug("%s path[%d].start = %d\n", __func__, i, path[i].start);
		pr_debug("%s path[%d].end = %d\n", __func__, i, path[i].end);
		pr_debug("%s path[%d].len = %d\n", __func__, i, path[i].len);
		pr_debug("%s path[%d].mid = %d\n", __func__, i, path[i].mid);
	}

finish:
	pr_debug("%s Final chosen phase: %d\n", __func__, final_phase);
	return final_phase;
}
...
static int rtk_sdmmc_change_tx_phase(struct rtk_sdmmc_host *rtk_host, u8 sample_point)
{
	...
	writeb(readb(rtk_host->sdmmc + SD_CONFIGURE1)&0xef, rtk_host->sdmmc + SD_CONFIGURE1);
	return 0;
}
...
static int rtk_sdmmc_tuning_tx(struct rtk_sdmmc_host *rtk_host)
{
	...
	for (i = 0 ; i < TUNING_CNT ; i++) {
		pr_debug("%s TX raw_phase_map[%d] = 0x%08x\n", __func__, i, raw_phase_map[i]);
...
static int rtk_sdmmc_tuning_rx(struct rtk_sdmmc_host *rtk_host)
{
	...
	phase_map = 0xFFFFFFFF;
	for (i = 0 ; i < TUNING_CNT ; i++) {
		pr_debug("%s RX raw_phase_map[%d] = 0x%08x\n", __func__, i, raw_phase_map[i]);
		phase_map &= raw_phase_map[i];
	}
...
static void rtk_sdmmc_speed(struct rtk_sdmmc_host *rtk_host, enum sdmmc_clock_speed sd_speed)
{
	...
	case SDMMC_CLOCK_50000KHZ:
		pr_debug("%s: speed SDMMC_CLOCK_50000KHZ\n", __func__);
	...
	case SDMMC_CLOCK_100000KHZ:
		pr_debug("%s: speed SDMMC_CLOCK_100000KHZ\n", __func__);

		if (chip_id == CHIP_ID_RTD1619B) {
			tmp = ((readl(isopad_base + 0x78) & 0xfe07f03f) | 0x00900480);
			writel(tmp, isopad_base + 0x78);
			tmp = ((readl(isopad_base + 0x7c) & 0xfff81fc0) | 0x00024012);
			writel(tmp, isopad_base + 0x7c);
			tmp = ((readl(isopad_base + 0x80) & 0xfe07f03f) | 0x00900480);
			writel(tmp, isopad_base+0x80);
		} else if (chip_id == CHIP_ID_RTD1319) {
	...
	case SDMMC_CLOCK_208000KHZ:
		pr_debug("%s: speed SDMMC_CLOCK_208000KHZ\n", __func__);
		...
		if (chip_id == CHIP_ID_RTD1619B) {
			...
			case 3:
			case 4:
				writel((readl(isopad_base+0x78) & 0xfe07f03f) | 0x00d806c0, isopad_base+0x78);
				writel((readl(isopad_base+0x7c) & 0xfff81fc0) | 0x0003601b, isopad_base+0x7c);
				writel((readl(isopad_base+0x80) & 0xfe07f03f) | 0x00d806c0, isopad_base+0x80);
				break;
			case 5:
			case 6:
			case 7:
				writel((readl(isopad_base+0x78) & 0xfe07f03f) | 0x01200900, isopad_base+0x78);
				writel((readl(isopad_base+0x7c) & 0xfff81fc0) | 0x00048024, isopad_base+0x7c);
				writel((readl(isopad_base+0x80) & 0xfe07f03f) | 0x01200900, isopad_base+0x80);
				break;
			case 8:
			case 9:
				writel((readl(isopad_base+0x78) & 0xfe07f03f) | 0x01680b40, isopad_base+0x78);
				writel((readl(isopad_base+0x7c) & 0xfff81fc0) | 0x0005a02d, isopad_base+0x7c);
				writel((readl(isopad_base+0x80) & 0xfe07f03f) | 0x01680b40, isopad_base+0x80);
				break;
			default:
				writel((readl(isopad_base+0x78) & 0xfe07f03f) | 0x00900480, isopad_base+0x78);
				writel((readl(isopad_base+0x7c) & 0xfff81fc0) | 0x00024012, isopad_base+0x7c);
				writel((readl(isopad_base+0x80) & 0xfe07f03f) | 0x00900480, isopad_base+0x80);
			}
		}
		...
		udelay(200);
		writel(0x00000006, pll_base + 0x01EC); //JIM modified, 1AC->1EC
		writel(0x00b64388, pll_base + CR_PLL_SD3); //SD clock rate formula: (ssc_div_n +3) *4.5/4
		mdelay(2);
		writel(0x00000007, pll_base + 0x01EC); //JIM modified, 1AC->1EC
		udelay(200);
		writel(0x00000006, pll_base + 0x01EC); //JIM modified, 1AC->1EC
		...
		break;
	default :
		pr_debug("%s: default speed SDMMC_CLOCK_400KHZ\n", __func__);
		rtk_sdmmc_set_div(rtk_host, CLOCK_DIV_256); //0x580 = 0xd0
		rtk_sdmmc_set_speed(rtk_host, 0); //0x478 = 0x2100
		break;
	}
...
int rtk_sdmmc_cpu_wait(char* drv_name, struct rtk_sdmmc_host *rtk_host, u8 cmdcode) {
	...
	while (time_before(jiffies, old_jiffles + timeout)) {
		...
		if ((readb(sdmmc_base + SD_TRANSFER) & (ERR_STATUS)) == (ERR_STATUS)) {
			...
			if (rtk_host->mrq != NULL && rtk_host->mrq->cmd != NULL)
...
int rtk_sdmmc_int_wait(char* drv_name, struct rtk_sdmmc_host *rtk_host, u8 cmdcode)
{
	...
	if(irq_error_bit==1) {      //error handling
		if (rtk_host->cmd_opcode==55 && CMD8_SD1==true) {
			...
			if(rtk_host!=NULL)
				pr_err("rtk_sdmmc_int_wait: reset the card and adjust the driving capacity, cmd=%d, SD_TRANSFER=0x%x\n",rtk_host->cmd_opcode, sd_trans);
			driving_capacity++;
...
static int rtk_sdmmc_stream_cmd(u16 cmdcode, struct sdmmc_cmd_pkt *cmd_info, u8 data_len)
{
	...
	rtk_host->cmd_opcode = cmd_idx;
	rtk_sdmmc_get_cmd_timeout(cmd_info);
	//pr_err("rtk_stream_cmd invoke pre_cmd=%d, cmd=%d, INT4_flag=%d\n",pre_cmd,cmd_info->cmd->opcode,INT4_flag);
	...
	if (ret == CR_TRANS_OK) {
		if ((cmdcode == SD_AUTOREAD1) || (cmdcode == SD_AUTOWRITE1)) {
			pr_debug("%s: auto read/write 1 skip response\n", __func__);
#if IS_ENABLED(CONFIG_CMD25_WO_STOP_COMMAND)
		} else if (cmdcode == SD_AUTOWRITE2) {
			pr_debug("%s: auto write 2 skip response\n", __func__); //CMD + DATA
		} else if (cmdcode == SD_AUTOWRITE3) {
			pr_debug("%s: auto write 3 skip response\n", __func__); //DATA only, clear rsp
#endif
		} else {
			rtk_sdmmc_read_rsp(rtk_host,rsp, rsp_len);
			pr_debug("%s: stream cmd done\n", __func__);
		}
	}
...
static int rtk_sdmmc_stream(struct sdmmc_cmd_pkt *cmd_info)
{
...
#if IS_ENABLED(CONFIG_CMD25_WO_STOP_COMMAND)
	/* send stop command only use arg info and we need to save it as global
	 * variable for fear that timer trigger stop command data corruption
	 */
	cmd12_stop_cmd.arg = cmd_info->cmd->arg;
	...
	if (rtk_host->sdmmc_bounce_buf_val) {
		int buflen = 0;

		for_each_sg(cmd_info->data->sg, sg, cmd_info->data->sg_len, i)
			buflen += sg->length;

		sg_init_one(rtk_host->sdmmc_bounce_sg,
			    rtk_host->sdmmc_bounce_buf,
			    buflen);

		if (dir == DMA_TO_DEVICE)
			sg_copy_to_buffer(cmd_info->data->sg,
					  cmd_info->data->sg_len,
					  rtk_host->sdmmc_bounce_buf,
					  rtk_host->sdmmc_bounce_sg[0].length);

		dma_nents = dma_map_sg(mmc_dev(host),
				       rtk_host->sdmmc_bounce_sg,
				       1,
				       dir);

		sg = rtk_host->sdmmc_bounce_sg;
	...
	for (i = 0 ; i < dma_nents ; i++, sg++) {
		...
		pr_debug("%s: dma_addr: 0x%x, dma_leng: 0x%x\n", __func__, dma_addr, dma_leng);

		if ((cmd_idx == SD_SWITCH) && ((cmd_info->cmd->flags & (0x3<<5)) == MMC_CMD_ADTC)) {
...
#if IS_ENABLED(CONFIG_CMD25_WO_STOP_COMMAND) //new write method
	...
	if (rtk_host->sdmmc_bounce_buf_val) {
		dma_unmap_sg(mmc_dev(host), rtk_host->sdmmc_bounce_sg, 1, dir);

		if (dir == DMA_FROM_DEVICE)
			sg_copy_from_buffer(cmd_info->data->sg,
					    cmd_info->data->sg_len,
					    rtk_host->sdmmc_bounce_buf,
					    rtk_host->sdmmc_bounce_sg[0].length);
	} else {
		dma_unmap_sg(mmc_dev(host),
			     cmd_info->data->sg,
			     cmd_info->data->sg_len,
			     dir);
	}
...
void rtk_sdmmc_open_clk(struct mmc_host *host)
{
	...
	if ((card_exist & 0x8)==0) {
		...
		writel(0x00000006, pll_base + 0x01EC);   //JIM modified, 1AC->1EC
		if (chip_id == CHIP_ID_RTD1619B)
			writel(0x0451942d, pll_base + CR_PLL_SD2);
		else
			writel(0x04519893, pll_base + CR_PLL_SD2); //change from 4517893 to 0x04519893 for passing the EMI
...
static int rtk_sdmmc_get_cd(struct mmc_host *host)
{
	...
	if (!(card_exist & 0x8)) {
		pr_err("%s: SD card exists, card_exist = 0x%x\n", __func__, card_exist);
		if(compatible_flag || (rtk_host->ins_event != EVENT_INSER && rtk_host->ins_event !=EVENT_REMOV) || re_initialize==true) {	//this two flags is to detect first recognition and error handling
			...
			rtk_sdmmc_speed(rtk_host, SDMMC_CLOCK_400KHZ);
			writeb(0x00, sdmmc_base + SD_STATUS2);
			pr_err("%s: SD card is being inserted now...!!!\n", __func__);
			rtk_sdmmc_sync(rtk_host);
	...
	pr_err("%s: SD card does not exist, card_exist = 0x%x\n", __func__, card_exist);
...
static int rtk_sdmmc_get_ro(struct mmc_host *host)
{
	...
	pr_err("%s: SD card is not write protect\n", __func__);
	...
	if (ios->bus_mode == MMC_BUSMODE_PUSHPULL) {
		...
		if (ios->clock >= UHS_SDR104_MAX_DTR && ios->timing == MMC_TIMING_UHS_SDR104) {
			...
			if (rtk_host->rtflags & RTKCR_FCARD_SELECTED) {
				rtk_sdmmc_speed(rtk_host, SDMMC_CLOCK_6200KHZ);
				pr_debug("%s: Mid speed RTKCR_FCARD_SELECTED = 1 SDMMC_CLOCK_6200KHZ\n", __func__);
			} else {
				rtk_sdmmc_speed(rtk_host, SDMMC_CLOCK_400KHZ);
				pr_debug("%s: Low speed SDMMC_CLOCK_400KHZ\n", __func__);
			}
		}
	} else {  //MMC_BUSMODE_OPENDRAIN
		pr_debug("%s: ios busmode != pushpull low speed SDMMC_CLOCK_400KHZ\n", __func__);
		rtk_sdmmc_speed(rtk_host, SDMMC_CLOCK_400KHZ);
		rtk_sdmmc_set_bits(rtk_host,BUS_WIDTH_1);
	}
...
static void rtk_sdmmc_timeout(struct timer_list *t)
{
	...
	pr_err("%s: command/data timeout, pre_cmd=%d, cmd=%d, poll=%d, ISR=%x, ISR_EN=%x\n", __func__,pre_cmd,rtk_host->cmd_opcode,poll,int_status,int_status_en);
...
static void rtk_sdmmc_card_power(struct rtk_sdmmc_host *rtk_host, u8 status)
{
	...
	if (status) {
		res = gpiod_direction_output(rtk_host->sdmmc_gpio, 0);

		if (chip_id == CHIP_ID_RTD1619B) {
			/* Set gpio pull down/up function to pull up */
			tmp = ((readl(isopad_base + 0x78) & 0xe3ff1ff8) | 0x1c00e007);
			writel(tmp, isopad_base + 0x78);
			tmp = ((readl(isopad_base + 0x7c) & 0xfffffc7f) | 0x00000380);
			writel(tmp, isopad_base + 0x7c);
			tmp = ((readl(isopad_base + 0x80) & 0xffff1ff8) | 0x0000e007);
			writel(tmp, isopad_base+0x80);
			/* Set pad driving */
			tmp = ((readl(isopad_base + 0x78) & 0xfe07f03f) | 0x00d806c0);
			writel(tmp, isopad_base + 0x78);
		...
		if (res < 0) {
			pr_err("%s: can't gpio output = %d (power on)\n", __func__, desc_to_gpio(rtk_host->sdmmc_gpio));
		} else {
			power_status = 1; //card is power on
			mdelay(10); //delay 10 ms after power on
		}
	} else {
		res = gpiod_direction_input(rtk_host->sdmmc_gpio);

		if (chip_id == CHIP_ID_RTD1619B) {
			writel(readl(isopad_base + 0x78) & 0xfbffdffe,isopad_base + 0x78);
			writel(readl(isopad_base + 0x7c) & 0xffffff7f,isopad_base + 0x7c);
			writel(readl(isopad_base + 0x80) & 0xffffdffe,isopad_base + 0x80);
		} else if (chip_id == CHIP_ID_RTD1319) {
...
static const struct of_device_id rtk_sdmmc_match[] = {
	{ .compatible = "realtek,rtd129x-sdmmc", },
...
#if IS_ENABLED(CONFIG_MMC_BLOCK_BOUNCE)
	pr_info("%s: CONFIG_MMC_BLOCK_BOUNCE enable\n", DRIVER_NAME);
#else
...
static int rtk_sdmmc_probe(struct platform_device *pdev)
{
	...
	const u32 *prop;
	struct gpio_desc *sd_power_gpio_tmp;
	struct gpio_desc *sd_wp_gpio_tmp;
	struct gpio_desc *sd_cd_gpio_tmp;
	int size;

	setup_chip_id();

	sd_power_gpio_tmp = devm_gpiod_get(&pdev->dev, "sd-power", GPIOD_OUT_LOW);
	if (IS_ERR(sd_power_gpio_tmp)) {
		dev_err(&pdev->dev, "can't request sd-power gpio\n");
		return -EPROBE_DEFER;
	}

	sd_wp_gpio_tmp = devm_gpiod_get(&pdev->dev, "sd-wp", GPIOD_IN);
	...
	if (IS_ERR(sd_cd_gpio_tmp)) {
		dev_err(&pdev->dev, "can't request sd-cd gpio\n");
		devm_gpiod_put(&pdev->dev, sd_power_gpio_tmp);
		devm_gpiod_put(&pdev->dev, sd_wp_gpio_tmp);
		return -EPROBE_DEFER;
	}
	...
	rtk_host->sdmmc = of_iomap(sdmmc_node, 0);
	rtk_host->pll = of_iomap(sdmmc_node, 1);
	...
	prop = of_get_property(pdev->dev.of_node, "sdmmc-bounce", &size);
	if (prop) {
		rtk_host->sdmmc_bounce_buf_val = of_read_number(prop, 1);
		pr_err("[%s] get sdmmc-bounce value : %d \n",__func__, rtk_host->sdmmc_bounce_buf_val);
	} else {
		rtk_host->sdmmc_bounce_buf_val = 0;
		pr_err("[%s] assign sdmmc-bounce default value %d !!\n",__func__, rtk_host->sdmmc_bounce_buf_val);
	}
	...
	if (chip_id == CHIP_ID_RTD1319)
		...
		writel(readl(rtk_host->gpiodir + 0xc4) | 0xc, rtk_host->gpiodir + 0xc4);
	...
	if(rtk_host->sdmmc_bounce_buf_val == 1)
		mmc->max_segs = 256;
	else
		mmc->max_segs = 1;
	...
	if(rtk_host->sdmmc_bounce_buf_val==1) {
		pr_err("Allocate SDMMC bounce buffer !!!\n");

		rtk_host->sdmmc_bounce_buf = kmalloc(BOUNCE_SIZE, GFP_KERNEL);
		if(!rtk_host->sdmmc_bounce_buf)
			pr_err("unable to allocate sdmmc_bounce_buffer !!!\n");

		rtk_host->sdmmc_bounce_sg = sdmmc_alloc_sg(1, GFP_KERNEL);
		if (!rtk_host->sdmmc_bounce_sg)
			pr_err("unable to allocate sdmmc_bounce_sg !!!\n");
	}
	...
	rtk_host->cd_irq = gpiod_to_irq(rtk_host->sdmmc_cd_gpio);
	if(!rtk_host->cd_irq)
...
static int rtk_sdmmc_remove(struct platform_device *pdev)
{
	...
	if (mmc) {
		...
		mmc_remove_host(mmc);
		if (!mmc) {
			pr_info("Realtek SD/MMC host have removed.\n");
		}
		...
		mmc_free_host(mmc);
		gpiod_put(rtk_host->sdmmc_gpio);
		if(rtk_host->sdmmc_bounce_buf_val==1) {
			pr_err("release SDMMC bounce buffer !!!\n");
			kfree(rtk_host->sdmmc_bounce_buf);
			kfree(rtk_host->sdmmc_bounce_sg);
		}
	}
...
static SIMPLE_DEV_PM_OPS(rtk_sdmmc_pm_ops, rtk_sdmmc_pm_suspend, rtk_sdmmc_pm_resume);
...
MODULE_DESCRIPTION("SD/MMC card driver for Realtek DHC SoCs");
MODULE_LICENSE("GPL");
